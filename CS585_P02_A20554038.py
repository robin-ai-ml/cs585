...
def main():
    ...
    df = pd.read_csv(
        csv_file_path, usecols=["Id", "Score", "Summary", "Text"], header=0
    )
    ...
    train_df, test_df = train_test_split(
        df, train_size=train_size, test_size=1 - train_size
    )
    ...
    #     prob_word_given_label_good = (
    #         len(
    #             train_df[
    #                 (train_df["Label"] == "good")
    #                 & (train_df["Text"].str.contains(word))
    #             ]
    #         )
    #         + smoothing_alpha / pre_calc_label_good_denominator
    #     )  # (count_label_good  + smoothing_alpha * count_label_good)

    #     prob_word_given_label_bad = (
    #         len(
    #             train_df[
    #                 (train_df["Label"] == "bad") & (train_df["Text"].str.contains(word))
    #             ]
    #         )
    #         + smoothing_alpha / pre_calc_label_bad_denominator
    ...
    for index, row in test_df[["Text", "Label"]].iterrows():
        ...
        predicted_label, _ = test(review.split(), logprior, loglikelihood, C, V)
        ...
        # predicted_label = (
        #     "good" if predict_prob_label_good > predict_prob_label_bad else "bad"
        # )
    ...
    while True:
        ...
        # classified_as, prob_good, prob_bad = classify_sentence(
        #     sentence,
        #     prob_label_good,
        #     prob_label_bad,
        #     prob_word_given_label_good_dict,
        ...
        classified_as, probs = test(sentence.split(), logprior, loglikelihood, C, V)
        print(
            f"\nSentence S:\n\n'{sentence}'\n\nwas classified as {classified_as}.\nP(good | S) = {probs['good']:.4f}\nP(bad | S) = {probs['bad']:.4f}\n"
        )
