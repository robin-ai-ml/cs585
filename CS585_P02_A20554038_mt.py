...
def calculate_probabilities(word_list, train_df, smoothing_alpha, pre_calc_label_good_denominator, pre_calc_label_bad_denominator):
        word_probs_good = {}
        word_probs_bad = {}

        for word in word_list:
            prob_word_given_label_good = len(train_df[(train_df['Label'] == 'good') & (train_df['Text'].str.contains(word))]) + smoothing_alpha \
                                        / pre_calc_label_good_denominator
            prob_word_given_label_bad = len(train_df[(train_df['Label'] == 'bad') & (train_df['Text'].str.contains(word))]) + smoothing_alpha \
                                        / pre_calc_label_bad_denominator

            word_probs_good[word] = prob_word_given_label_good
            word_probs_bad[word] = prob_word_given_label_bad

        return word_probs_good, word_probs_bad
...
def main():
    ...
    # Split the vocabulary into chunks
    num_threads = mp.cpu_count()  # Get the number of available CPU cores
    vocab_chunks = [[] for _ in range(num_threads)]
    for i, word in enumerate(vocab_in_training):
        vocab_chunks[i % num_threads].append(word)

    # Create a pool of worker processes
    pool = mp.Pool(processes=num_threads)

    # Calculate probabilities in parallel
    results = pool.starmap(calculate_probabilities, [(chunk, train_df, smoothing_alpha, pre_calc_label_good_denominator, 
                                                      pre_calc_label_bad_denominator) for chunk in vocab_chunks])

    # Merge the results
    prob_word_given_label_good_dict = {}
    prob_word_given_label_bad_dict = {}
    for good_dict, bad_dict in results:
        prob_word_given_label_good_dict.update(good_dict)
        prob_word_given_label_bad_dict.update(bad_dict)
    ...
    end_time = time.time()
    print("---  training in seconds --- %.2f" % (end_time - start_time ))
    ...
    while True:
        ...
        sentence = expand_contractions(sentence, contractions_map)
        sentence = remove_punctuation(sentence)
        sentence = remove_stopwords(sentence)
        sentence = stem_text(sentence)
        sentence = lemmatize_text(sentence)
        sentence = remove_numbers(sentence)
        sentence = remove_single_character(sentence)
        ...
        print(f"\nIt is classified as {classified_as}.\nP(good | S) = {prob_good:.4f}\nP(bad | S) = {prob_bad:.4f}\n")
